...
int main(int argc, char *argv[]) {
  ...
  /*save = omp_get_num_threads();*/

  y = ymin + (tid*(h/nb_threads)) * yinc;

  for (int i = tid * (h/nb_threads) ; i < (tid+1) * (h/nb_threads); i++) {	
    x = xmin;
    for (int j = 0; j < w; j++) {
       /*printf("%d\n", xy2color( x, y, prof));*/
  ...
  /*fprintf( stderr, "Temps total de calcul : %g sec\n", */
		 /*fin - debut);*/
  t_1 = 3.41558;
  fprintf( stdout, "%d %g %g %g\n",nb_threads,fin - debut,t_1/(fin-debut),t_1/(nb_threads*(fin-debut)));
